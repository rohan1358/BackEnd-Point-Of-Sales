...
module.exports={
    getProduct: () => {
        return new Promise((resolve, reject) => {
            connecttion.query("SELECT product.*, categori.name AS categori FROM product INNER JOIN categori ON product.id_categori = categori.id", (err, result) => {
                if(!err){
                    resolve(result);
                }else{
    ...
    productDetail: (id_product) => {
        return new Promise((resolve, reject) => {
            connecttion.query(`SELECT * FROM product WHERE id = ${id_product}`, (err, result) => {
                if(!err){
    ...
    pageProduct:(page)=>{
        return new Promise((resolve, reject) => {
        const batas = parseInt(page)
        const limit = 1
        const index = limit*(batas - 1)
            connecttion.query("SELECT * FROM product LIMIT ? OFFSET ?",[limit, index], (err, result)=>{
                if(!err){
                    resolve(result);
                }else{
                    reject(new Error(err));
    ...
    filterProduct:(filter)=>{
        ...
        return new Promise((resolve, reject) => {
            if(filter === "string"){
                connecttion.query("SELECT * FROM product WHERE price = ?", filter, (err, result) => {
                    if(!err){
                        resolve(result)
                    }else{
                        reject(err)
                ...
                connecttion.query("SELECT * FROM product WHERE id_categori = ?", filter, (err, result) => {
                    if(!err){
                        resolve(result)
                    }else{
                        reject(err)
